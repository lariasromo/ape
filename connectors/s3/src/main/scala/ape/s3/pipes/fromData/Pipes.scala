...
protected[s3] class Pipes[Config <: S3Config :Tag]() {
  class encoded[ZE] {
    ...
    Pipe[ZE with S3 with Config, ZE, T, S3ObjectSummary] = new AvroPipe[ZE, T, Config]
//   todo: make parquet work with IAM roles
...
//      new ParquetPipe[ZE, T, Config](chunkSize, duration)
  ...
  class text[ZE] {
    def default: Pipe[ZE with S3 with Config, ZE, String, S3ObjectSummary] = new TextPipe[ZE, Config]
    def csv[T: ClassTag](
                          sep: String = ",",
                          order:Option[Seq[String]]=None
                        ): Pipe[ZE with S3 with Config, ZE, T, S3ObjectSummary] = new CsvPipe[ZE, T, Config](sep, order)
  }
  def text[ZE](implicit d1: ZE := Any) = new text[ZE]

  class jsonLines[ZE] {
    def default[T: ClassTag](implicit e: T => String): Pipe[ZE with S3 with Config, ZE, T, S3ObjectSummary] =
      new JsonLinesPipe[ZE, T, Config]

    def circe[T: io.circe.Encoder: ClassTag]: Pipe[ZE with S3 with Config, ZE, T, S3ObjectSummary] =
