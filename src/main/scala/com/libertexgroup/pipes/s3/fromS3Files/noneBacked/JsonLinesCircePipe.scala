...
import com.libertexgroup.pipes.s3.fromS3Files.{S3ContentPipe, S3FilePipe}
...
class JsonLinesCircePipe[
  ...
  override protected[this] def pipe(i: ZStream[Any, Throwable, S3ObjectSummary]):
    ZIO[Config, Throwable, ZStream[Any, Throwable, S3FileWithContent[T]]] = S3FilePipe.jsonLinesCircePipe(i)
