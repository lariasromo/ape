...
object AvroUtils {
  ...
  object implicits {
    ...
    implicit val chunkToSeq: Chunk[Byte] => DecoderImplicit = ba => new DecoderImplicit(ba.toArray)
  }
  ...
  class EncoderImplicit(data: Seq[Any]) {
    def encode[T: SchemaFor : Encoder](): Option[Array[Byte]] = {
      ...
      Try(AvroOutputStream.binary[T].to(baos).build()) match {
        case Failure(exception) => {
          exception.printStackTrace()
          None
        }
        case Success(os) => {
          val records = data.map(_.asInstanceOf[T])
          os.write(records)
          os.flush()
          os.close()
